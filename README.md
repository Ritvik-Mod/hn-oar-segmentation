...
| Property | Details |
|---|---|
| **Total patients** | 844 confirmed H&N radiotherapy patients |
| **Institution** | Single Indian tertiary cancer centre |
| **Treatment Planning System** | Elekta Monaco TPS |
| **Raw data formats** | 797 patients in proprietary `.WC` format + 117 patients in DICOM-RT format |
| **Usable patient folders** | 914 (after quality filtering) |
| **Annotated slices** | 126,879 CT slices |
...
The majority of the dataset (797 patients) existed exclusively in Elekta Monaco's proprietary `.WC` file format, for which **no public documentation, parser, or specification exists**. The format was reverse engineered entirely from first principles through binary analysis, pattern recognition across multiple patient files, and iterative hypothesis testing.
...
- A **label standardisation pass** was run across all 126,879 slice files with zero labelling errors
...
Five architectures are evaluated under **strictly controlled, identical conditions** — same loss function, same optimiser, same hyperparameters, same data split. This is a controlled benchmark, not a model selection exercise.
...
**Training protocol (identical across all five):**
- Loss: `0.5 × Dice + 0.5 × BCE` (with sigmoid activation)
- Optimiser: Adam, lr = 1e-4
- Early stopping: patience = 10 epochs
- Data split: same locked 583/84/165 patient-wise split

**Status:** U-Net and Attention U-Net training complete. TransUNet in progress.

> Quantitative results (Dice scores, HD95 values, etc.) are withheld pending publication.
...
| **Clinical Tversky (α=0.3, β=0.7)** | Custom asymmetric loss metric penalising under segmentation more heavily than over segmentation — under segmentation exposes healthy tissue to radiation dose |
...
Experiment:    (logging framework — details withheld)
Hardware:      (compute details withheld)
```
...
This project uses retrospective clinical CT and radiotherapy planning data from an Indian tertiary cancer centre, collected under institutional ethics review. No patient-identifiable information is present in any processed file. Dataset is not publicly shareable.
