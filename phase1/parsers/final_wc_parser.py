...
# --- CONFIG ---
parent_data_dir = '/Users/ritvikmod/Desktop/patient data'
...
# --- BATCH EXECUTION ---
patient_folders = [f.path for f in os.scandir(parent_data_dir) if f.is_dir()]
print(f"Found {len(patient_folders)} patients in {parent_data_dir}")
...
for p_path in patient_folders:
    ...
    for ct_path in ct_folders:
        ...
        for wc_file in all_wc:
            ...
            if z_c not in dcm_dict: continue
            
            try:
                ds = pydicom.dcmread(dcm_dict[z_c], force=True)
                ...
                try: image_array = ds.pixel_array
                    ...
                    with open(dcm_dict[z_c], 'rb') as f:
                        f.seek(os.path.getsize(dcm_dict[z_c]) - (rows*cols*2))
