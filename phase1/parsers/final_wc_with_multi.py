...
def process_patient(p_path):
    ...
    # --- RESUME LOGIC ---
    # If the folder exists, it was already processed in the first 340. Skip it instantly!
    if os.path.exists(os.path.join(master_ml_output, p_id)):
    ...
    if not ct_folders:
        return f"Skipped: {p_id} (No valid CT1 folder)"
        ...
        for wc_file in all_wc:
            ...
            try:
                ...
                if slice_masks:
                    ...
                    # Matplotlib thread safety for multiprocessing
    ...
    return f"Finished Processing: {p_id}"

# --- MULTIPROCESSING TRIGGER ---
if __name__ == '__main__':
    print(f"Kicking off Multiprocessing for {len(patient_folders)} patients...")
    
    # 8 parallel workers will chew through this flawlessly without freezing your system.
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        results = executor.map(process_patient, patient_folders)
        
        # This keeps your log file updating so you can still monitor progress
        for res in results:
            if res:
                print(res)
