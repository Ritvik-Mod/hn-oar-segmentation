...
def main():
    print("Loading Attention U-Net weights for full clinical evaluation...")
    device = torch.device('cpu')
    print(f"Using device: {device}")

    model_name = "Attention_UNet"
    model = AttentionUNet(in_channels=1, out_channels=2).to(device)
    weights_path = '/home/<hpc-user>/project/checkpoints/attention_unet/best_model.pth'
    ...
    with h5py.File(dataset_path, 'r') as hf:
        for i, patient_id in enumerate(val_patients):
            ...
            for idx, slice_name in enumerate(slice_names):
                if idx % 20 == 0:
                    print(f"  Slice {idx+1}/{len(slice_names)}", flush=True)
                ...
                with torch.no_grad():
                    ...
                    pred_prob = torch.sigmoid(output).squeeze(0).cpu().numpy()

                vol_pred_r.append((pred_prob[0] > 0.5).astype(np.float32))
                vol_true_r.append(mask_r)
                vol_pred_l.append((pred_prob[1] > 0.5).astype(np.float32))
