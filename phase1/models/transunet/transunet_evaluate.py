...
def main():
    print("Loading TransUNet weights for full clinical evaluation...")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")

    model_name = "TransUNet"
    model = TransUNet(in_channels=1, out_channels=2).to(device)
    weights_path = '/home/<hpc-user>/project/checkpoints/transunet/best_model.pth'
    ...
    # Custom exception to break out of visititems instantly
    ...
    with h5py.File(dataset_path, 'r') as hf:
        for i, patient_id in enumerate(val_patients):
            ...
            # ── OPTIMIZATION 2: CPU Batch Processing (8 slices at a time) ──
            BATCH_SIZE = 8
